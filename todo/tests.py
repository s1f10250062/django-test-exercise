...
class TodoViewTestCase(TestCase):
    ...
    def test_index_get_search(self):
        task1 = Task(title='search target', due_at=timezone.make_aware(datetime(2024, 7, 1)))
        task1.save()
        task2 = Task(title='another task', due_at=timezone.make_aware(datetime(2024, 8, 1)))
        task2.save()
        client = Client()
        response = client.get('/?q=target')
